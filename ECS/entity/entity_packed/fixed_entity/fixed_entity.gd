...
## 架构设计：
## - 继承自 [IEntity] 基类
## - 支持 [signal entity_ray_interact] 射线交互
## - 与 [Main] 系统的实体初始化集成
## - 基于 [SavedDataFile] 的存档系统支持
##
## [br][b]编辑者:[/b] Sora
@tool
...
## 实体射线交互信号
## 
## 用于处理基于 [RayCast2D] 的实体间交互，主要用于玩家与环境对象的交互。
## [param interact_source]: 发起交互的实体，类型为 [IEntity]
...
## [param _data]: 存档数据文件对象，类型为 [SavedDataFile]（当前未使用，为将来扩展预留）
## [br][br][b]返回:[/b] [Dictionary] 包含实体完整信息的存档字典
...
## 从存档数据中恢复实体及其组件的状态。
## [param _data]: 存档数据文件对象，类型为 [SavedDataFile]（当前未使用，为将来扩展预留）
## [br][br][b]返回:[/b] [Dictionary] 包含组件加载数据的字典
