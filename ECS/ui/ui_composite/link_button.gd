...
## 该按钮提供两种界面操作模式：Creation模式（动态创建新的UI界面）、Linkage模式（链接到已存在的UI组件）
## 主要功能：智能的UI界面管理、动态场景实例化、编辑器属性的条件显示、目标界面的生命周期管理
## 使用场景：主菜单的设置按钮、游戏内的子界面调用、模态对话框的打开、多窗口应用的界面切换
## 架构设计：继承自 [Button] 基类，基于 [enum LinkMode] 的模式管理，与 [PackedScene] 和 [Control] 集成
## 编辑器集成：[annotation @tool] 标记支持编辑器预览、条件属性显示优化工作流、实时的模式切换反馈
## [br][b]编辑者:[/b] Sora
@tool
