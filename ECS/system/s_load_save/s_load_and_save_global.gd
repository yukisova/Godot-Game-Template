## 存档系统 - 管理游戏数据的保存和加载机制
## 负责游戏状态的持久化存储，包括实体状态、组件数据、系统配置等
## 存档范围：全局黑板数据、地图数据和实体状态、游戏运行时缓存数据、系统配置信息
## 功能特性：异步存档机制、递归数据收集、版本兼容性支持、数据完整性验证
## [br][b]编辑者:[/b] Sora
extends ISystem

## 游戏保存开始信号，触发系统开始收集和保存所有可存档数据
@warning_ignore("unused_signal")
signal saving_started

## 游戏加载开始信号，触发系统开始加载存档数据
signal loading_started

## 当前存档数据，保存最近加载的存档文件引用，用于数据追踪和调试
var current_saved: SavedDataFile = null

## 节点初始化，连接存档和读档信号到对应的处理方法
func _enter_tree() -> void:
	saving_started.connect(_data_saving)
	loading_started.connect(_data_loading)

## 系统重置，清空运行时缓存数据，准备新的游戏会话
func _resetup():
	current_saved = null

## 执行数据保存，递归收集所有系统的可存档数据并保存到文件
func _data_saving():
	print("存档系统: 开始保存游戏数据...")
