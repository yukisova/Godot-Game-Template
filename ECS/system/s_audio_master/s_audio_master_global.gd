...
## 该系统提供统一的音频管理接口，支持背景音乐的无缝淡入淡出切换、多个音效播放器的管理
## 音频总线音量控制、游戏暂停时的音频处理
## 音频架构：Master总线（控制整体音量）、Music总线（背景音乐专用）、SFX总线（音效专用）
## 核心特性：双背景音乐播放器实现无缝切换、多音效播放器避免音效重叠问题
## 技术实现：使用 [Tween] 实现音量淡入淡出、基于 [AudioServer] 的音频总线管理
## 架构设计：继承自 [ISystem] 基类，基于 [enum AudioBusEnum] 的总线类型管理
## [br][b]编辑者:[/b] Sora
extends ISystem
