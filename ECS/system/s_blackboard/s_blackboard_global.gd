...
## 功能特性：全局数据存储和访问、数据变化事件通知、子系统管理、类型安全的数据操作
## 架构设计：基于 [Dictionary] 的数据存储、信号驱动的变化通知机制、子系统注册和管理
## [br][b]编辑者:[/b] Sora
extends ISystem
...
## 
## 将黑板中的重要数据保存到存档文件。
## [param _data]: 存档数据文件，参见 [SavedDataFile]
...
## 提供数据查询接口，支持按键名查找数据
## [br][br][b]返回:[/b] 查询结果，如果没有找到则返回null
