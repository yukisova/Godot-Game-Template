...
## 跟随策略类型：平滑跟随、直接跟随、预测跟随、鼠标辅助跟随
## 功能特性：策略模式的相机控制、相机边界限制、与地图系统集成、平滑相机过渡、可配置的跟随参数
## 架构设计：基于 [CameraFollowStrategy] 的策略模式、与 [SMapData] 系统的地图边界集成、使用 [Camera2D] 作为实际相机节点
## [br][b]编辑者:[/b] Sora
@tool
class_name CCamera
extends IComponent
...
## 设置相机边界限制，根据关卡数据设置相机的移动边界
## [param limit_dict]: 包含边界信息的字典，包含camera_top、camera_bottom、camera_left、camera_right字段
