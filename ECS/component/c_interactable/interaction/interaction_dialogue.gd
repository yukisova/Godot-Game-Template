...
## 
## 核心功能：
## - [DialogueManager] 集成：使用项目的对话管理器系统
...
## 工作流程：
## 1. 玩家触发对话交互
## 2. 通过 [SUiSpawner] 生成对话UI界面
## 3. 启动 [DialogueManager] 处理对话逻辑
## 4. 传递上下文信息和目标实体数据
## 5. 游戏进入对话状态，限制玩家操作
## 
## 应用场景：
...
## 架构设计：
## - 继承自 [Interaction] 基类
## - 集成 [PackedScene] 的UI管理
## - 基于 [DialogueResource] 的对话内容
## - 使用 [Dictionary] 的上下文数据传递
## - 与 [SoraEvent] 工具类的集成
##
## [br][b]编辑者:[/b] Sora
...
## 对话UI场景
## 
## 用于显示对话内容的UI界面场景，类型为 [PackedScene]。
@export var test_dialogue_ui: PackedScene

## 对话资源
## 
## 包含对话内容、选项和逻辑的对话资源文件，类型为 [DialogueResource]。
@export var test_dialogue: DialogueResource

## 对话标签
## 
## 对话资源中的起始标签或节点名称，类型为 [StringName]。
@export var test_dialogue_label: StringName

## 对话上下文信息
## 
## 传递给对话系统的额外数据和配置信息，类型为 [Dictionary] of [String] to [Variant]。
@export var dialogue_info: Dictionary[String, Variant]

## 交互激活处理（重写方法）
## 
## 当对话交互被触发时启动对话系统。
## [param _target_entity]: 触发交互的目标实体（通常是玩家），类型为 [IEntity]
