...
##
## 交互类型说明：
## - [constant InteractType.Null]: 禁用的交互（-1）
## - [constant InteractType.BodyEntered]: 刚体进入触发交互（0）
## - [constant InteractType.AreaEntered]: 区域进入触发交互（1）
## - [constant InteractType.RayCasted]: 射线检测触发交互（2）
##
## 功能特性：
## - 可视化编辑器配置
## - 动态属性验证
...
## 定义不同的交互触发方式和检测机制。
enum InteractType { 
	Null = -1,        ## 禁用交互 - 不响应任何触发条件
	BodyEntered = 0,  ## 刚体进入触发 - 当 [CharacterBody2D] 等刚体进入检测区域
	AreaEntered,      ## 区域进入触发 - 当 [Area2D] 区域重叠时
	RayCasted         ## 射线检测触发 - 基于射线检测的交互
}
...
## 交互类型
## 
## 决定交互的触发方式和检测机制，详见 [enum InteractType]。
...
## 交互逻辑节点路径
## 
## 指向 [Interaction] 类型的节点，定义交互的具体逻辑实现。
@export_node_path("Interaction") var interaction: NodePath

## 交互检测区域节点路径
## 
## 指向 [InteractBox] 类型的节点，定义交互的触发区域范围。
@export_node_path("InteractBox") var interact_box: NodePath

## 构造函数
## 
## 创建新的交互记录，设置初始配置参数。
## [param _interact_type]: 交互类型，默认为 [constant InteractType.BodyEntered]
## [param _is_passive]: 是否为被动交互，默认为false
