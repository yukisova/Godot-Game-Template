...
## 移动策略组件
## 
## 用于获取移动向量和方向信息，类型为 [MoveStrategy]。
@export var vector_move: MoveStrategy

## 纹理组件
## 
## 用于播放移动动画和控制角色视觉表现，类型为 [CTexture]。
...
func _enter():
	...
	# TODO: 根据移动方向播放对应的移动动画
	if c_texture and c_texture.animation_player:
		# c_texture.animation_player.play("move")
		pass
