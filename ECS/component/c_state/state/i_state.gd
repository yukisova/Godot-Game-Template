...
##
## 状态生命周期：
## 1. [method _enter]: 进入状态时执行
## 2. [method _update]: 状态活跃时每帧执行
## 3. [method _fixed_update]: 状态活跃时每物理帧执行
## 4. [method _blur_update]: 状态非活跃但仍在栈中时执行
## 5. [method _exit]: 离开状态时执行
##
## 状态控制：
## - [method _pause]: 暂停状态更新
## - [method _continue]: 恢复状态更新
##
## 功能特性：
## - 标准生命周期管理
## - 暂停/恢复机制
